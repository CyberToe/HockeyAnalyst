...
router.get('/games/:gameId', async (req, res, next) => {
    try {
        ...
        const shots = await prisma_1.prisma.shot.findMany({
            where: { gameId },
            include: {
                shooter: true,
                period: true,
...
// Record new shot
router.post('/games/:gameId', (0, validation_1.validateSchema)(validation_1.createShotSchema), async (req, res, next) => {
    try {
        const { gameId } = req.params;
        ...
        if (shooterPlayerId) {
            const shooter = await prisma_1.prisma.player.findUnique({
        ...
        const shot = await prisma_1.prisma.shot.create({
            data: {
                gameId,
                periodId,
                shooterPlayerId,
...
// Update shot
router.put('/:shotId', (0, validation_1.validateSchema)(validation_1.updateShotSchema), async (req, res, next) => {
    try {
        const { shotId } = req.params;
        const { shooterPlayerId, xCoord, yCoord, scored, scoredAgainst, notes } = req.body;
        // Get shot and verify team membership
        const shot = await prisma_1.prisma.shot.findUnique({
            where: { id: shotId },
            include: {
                game: {
                    include: { team: true }
        ...
        const updatedShot = await prisma_1.prisma.shot.update({
            where: { id: shotId },
            data: {
...
// Delete shot
router.delete('/:shotId', async (req, res, next) => {
    try {
        const { shotId } = req.params;
        // Get shot and verify team membership
        const shot = await prisma_1.prisma.shot.findUnique({
        ...
        await prisma_1.prisma.shot.delete({
            where: { id: shotId }
        });
...
router.delete('/games/:gameId/all', async (req, res, next) => {
    try {
        ...
        const result = await prisma_1.prisma.shot.deleteMany({
            where: { gameId }
        });
        res.json({
...
router.delete('/periods/:periodId/all', async (req, res, next) => {
    try {
...
// Get shot details
router.get('/:shotId', async (req, res, next) => {
    try {
        const { shotId } = req.params;
        const shot = await prisma_1.prisma.shot.findUnique({
            where: { id: shotId },
